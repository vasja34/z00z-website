...
**1. Filter file list:** Exclude non-source files unless the workflow explicitly requests planning-artifact review:
...
If the config includes `allow_planning_artifacts: true`, keep the provided file list as-is and skip the planning-artifact exclusions above. This is reserved for explicit user requests that target planning outputs directly.
