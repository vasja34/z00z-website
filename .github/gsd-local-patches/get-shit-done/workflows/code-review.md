...
Set explicit planning-artifact allowance for manual file scopes:
```bash
ALLOW_PLANNING_ARTIFACTS="false"
if [ -n "$FILES_OVERRIDE" ]; then
  ALLOW_PLANNING_ARTIFACTS="true"
fi
```
</step>
...
1. **Apply exclusions (per D-03) unless an explicit file list was provided:** Remove paths matching planning artifacts
```bash
if [ -z "$FILES_OVERRIDE" ]; then
  FILTERED_FILES=()
...
allow_planning_artifacts: ${ALLOW_PLANNING_ARTIFACTS}
