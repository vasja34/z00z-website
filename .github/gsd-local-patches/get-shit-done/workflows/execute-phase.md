...
@~/.copilot/get-shit-done/references/agent-contracts.md
@~/.copilot/get-shit-done/references/context-budget.md
@~/.copilot/get-shit-done/references/gates.md
</required_reading>
...
- Executor agents omit extended deviation rule examples and checkpoint examples from inline prompt — load on-demand via @~/.copilot/get-shit-done/references/executor-examples.md
- Planner agents omit extended anti-pattern lists and specificity examples from inline prompt — load on-demand via @~/.copilot/get-shit-done/references/planner-antipatterns.md
...
2. For each plan (sequentially, ignoring wave grouping):
   ...
   c. **If "Execute":** Read and follow `~/.copilot/get-shit-done/workflows/execute-plan.md` **inline**
   ...
   **Sequential dispatch for parallel execution (waves with 2+ agents):**
   ...
   Task(
     ...
     prompt="
       ...
       ${CONTEXT_WINDOW < 200000 ? '' : '@~/.copilot/get-shit-done/references/executor-examples.md'}
       </execution_context>
...
**This step is REQUIRED and must not be skipped.** Auto-invoke code review on the phase's source changes, then auto-invoke `gsd-code-review-fix` if the review is non-clean.
...
If REVIEW_STATUS is not "clean" and not "skipped" and not empty, **automatically** invoke:
```
Skill(skill="gsd-code-review-fix", args="${PHASE_NUMBER}")
```

**Error handling:** If either Skill invocation fails or throws, catch the error, display "Code review gate encountered an error (non-blocking): {error}" and proceed to next step. Review failures must never block execution.
...
Read and follow `~/.copilot/get-shit-done/workflows/transition.md`, passing through the `--auto` flag so it propagates to the next phase invocation.
