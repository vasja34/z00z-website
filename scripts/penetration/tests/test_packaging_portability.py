...
class PackagingPortabilityTest(unittest.TestCase):
    ...
    def test_pack_archive_includes_pentest_sources_and_excludes_heavy_payloads(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            run_command("bash", str(ROOT / "pack_z00z_website_project.sh"), "--output", str(archive_path))
    ...
    def test_entrypoint_and_unpack_help_expose_portable_contract(self) -> None:
        ...
        unpack_help = run_command("bash", str(ROOT / "unpack_z00z_website_project.sh"), "--help").stdout
        self.assertIn("--skip-verify", unpack_help)

    def test_unpack_surface_stays_website_only(self) -> None:
        unpack_script = (ROOT / "unpack_z00z_website_project.sh").read_text(encoding="utf-8")
        unpack_dockerfile = (ROOT / "config" / "docker" / "unpack.Dockerfile").read_text(encoding="utf-8")

        self.assertIn("Installing website dependencies with npm", unpack_script)
        self.assertNotIn("install_pentest_tools.sh", unpack_script)
        self.assertNotIn("check_pentest_tools.sh", unpack_script)

        lowered = unpack_dockerfile.lower()
        self.assertIn("from node:22-bookworm-slim", lowered)
        for forbidden in ("cargo", "rustc", "pipx", "nmap"):
