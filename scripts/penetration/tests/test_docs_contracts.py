...
class DocsContractsTest(unittest.TestCase):
    ...
    def test_verification_versions_env_stays_single_repo_source_of_truth(self) -> None:
        if not VERIFICATION_INSTALLER.exists() or not VERIFICATION_VERSIONS.exists():
            self.skipTest("verification-tools surface is not part of this repository")
